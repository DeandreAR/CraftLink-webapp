...
/**
 * Masque alpha composite : bords fondus vers le fond crème (transparents).
 */
...
/** Illustration hero : profil social → page CraftLink, bords fondus vers le crème. */
export function HeroAsideShowcase({ alt, className = "" }: HeroAsideShowcaseProps) {
  return (
    ...
    >
      <div className="landing-hero-aside-frame relative w-full max-w-[min(100%,56rem)] overflow-visible border-0 bg-transparent lg:max-w-[58rem]">
        ...
        {/* Voile crème : fondu transparent vers le fond de la section */}
        <div aria-hidden className="pointer-events-none absolute inset-0">
          <div
            className="absolute inset-y-0 left-0 w-[20%]"
            style={{
              background: `linear-gradient(to right, ${CREAM} 0%, color-mix(in srgb, ${CREAM} 35%, transparent) 55%, transparent 100%)`,
            }}
          />
          <div
            className="absolute inset-y-0 right-0 w-[14%]"
            style={{
              background: `linear-gradient(to left, ${CREAM} 0%, color-mix(in srgb, ${CREAM} 30%, transparent) 55%, transparent 100%)`,
            }}
          />
          <div
            className="absolute inset-x-0 top-0 h-[7%]"
            style={{
              background: `linear-gradient(to bottom, ${CREAM} 0%, color-mix(in srgb, ${CREAM} 35%, transparent) 55%, transparent 100%)`,
            }}
          />
          <div
            className="absolute inset-x-0 bottom-0 h-[16%]"
            style={{
              background: `radial-gradient(ellipse 118% 95% at 50% 100%, ${CREAM} 0%, color-mix(in srgb, ${CREAM} 50%, transparent) 45%, transparent 80%)`,
            }}
          />
        </div>
      </div>
