...
export function VitrinePhotoUpload({
  copy,
  onChange,
  maxPhotos = 3,
}: VitrinePhotoUploadProps) {
  ...
  const validateAndSet = useCallback(
    (incoming: FileList | File[]) => {
      ...
      const merged = [...files, ...list].slice(0, maxPhotos);
      if (files.length + list.length > maxPhotos) {
        setError(copy.photos.tooMany.replace("3", String(maxPhotos)));
      }

      setFiles(merged);
      onChange(merged);
    },
    [copy.photos, files, maxPhotos, onChange],
  );
